...
```
PIPL 审查 → 需要外发样本 → legal-document-redaction（生成脱敏样本 + 比对文件）
          → 外发第三方审查 / 监管报送
          → 收回审查反馈
...
- **触发时机**：PIPL/GDPR 合规审查需外发样本、监管报送需脱敏版本
- **输出用途**：第三方审查版 / 监管报送版 / 合规档案版
- **配置**：调用前 Read `legal-document-redaction/references/config.md`，健康数据强制细粒度+不可逆
- **法域对应**：cn-mainland → PIPL § 73 / hk → DPAO 1 / sg → PDPA § 2 / eu → GDPR Recital 26
