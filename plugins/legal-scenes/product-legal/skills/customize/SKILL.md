...
>
> **核心变更**：配置路径从 `~/.claude/plugins/config/claude-for-legal/product-legal/CLAUDE.md` 改为 `plugins/legal-scenes/product-legal/CLAUDE.md` § B9。集成 / 法规全部替换为 CN 工具与法规。
...
1. **读取配置。** 读 `plugins/legal-scenes/product-legal/CLAUDE.md`（特别是 § B9 用户配置）和 `plugins/shared/cold-start-interview/references/company-profile.md`（如果存在）。如果插件配置不存在或仍包含 `[填空]` 占位符，说：
