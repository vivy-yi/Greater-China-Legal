...
> **目的**：审计 Greater China Legal 项目 30 个 scene 的 `plugins/scenes/<scene>/CLAUDE.md` 是否符合 `anthropics/claude-for-legal` 上游 18 个 pattern + 4d 章"4 处位置"设计原则。
...
**GCL 现状**：`plugins/scenes/<scene>/CLAUDE.md` 顶部是 `[填空]`——**用户填了就被 git 跟踪**。
...
| **3 月** | 完整迁移 30 scene，删 `plugins/scenes/` |
